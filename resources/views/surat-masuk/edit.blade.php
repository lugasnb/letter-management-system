                ...
                <div class="p-6">
                    <form action="{{ route('surat-masuk.update', $suratMasuk) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="tanggal_diterima" id="tanggal_diterima" value="{{ old('tanggal_diterima', $suratMasuk->tanggal_diterima->format('Y-m-d')) }}"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('tanggal_diterima') border-red-500 @enderror">
                        ...
                        <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2" for="pengirim">
                                Pengirim <span class="text-red-500">*</span>
                            </label>
                            <input type="text" name="pengirim" id="pengirim" value="{{ old('pengirim', $suratMasuk->pengirim) }}"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('pengirim') border-red-500 @enderror">
                            @error('pengirim')
                                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="keterangan" id="keterangan" rows="4"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('keterangan') border-red-500 @enderror">{{ old('keterangan', $suratMasuk->keterangan) }}</textarea>
                            @error('keterangan')
                                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="belum_dibaca" {{ old('status', $suratMasuk->status) == 'belum_dibaca' ? 'selected' : '' }}>Belum Dibaca</option>
                                <option value="sudah_dibaca" {{ old('status', $suratMasuk->status) == 'sudah_dibaca' ? 'selected' : '' }}>Sudah Dibaca</option>
                        ...
                        <div class="mb-4">
                            ...
                            @if($suratMasuk->file_surat)
                                <p class="text-sm text-gray-600 mb-2">File saat ini: 
                                    <a href="{{ Storage::url($suratMasuk->file_surat) }}" target="_blank" class="text-blue-600 hover:underline">Lihat File</a>
                                </p>
                            @endif
                        ...
                        <div class="flex items-center justify-between mt-6">
                            <a href="{{ route('surat-masuk.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                                Kembali
                            </a>
